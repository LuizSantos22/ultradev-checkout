/**
 * UltraCheckout — OpenMage / Magento 1.x
 * Namespace: UltraCheckout
 * Depende de: UltraCheckoutConfig (injetado pelo root.phtml)
 */
var UltraCheckout = (function () {
    ...
    function calcProgress() {
        ...
        var tax   = tipoPessoa === 'pf'
                    ? digits('tax_document')
                    : digits('cnpj');
        var taxMin= tipoPessoa === 'pf' ? 11 : 14;
        var tel   = digits('telephone');
        var cep   = digits('postcode');
        var rua   = val('street');
        var num   = val('number');
        ...
        var p3 = 0;
        if (p2 >= 1 && lastPaymentTime > 0 && (Date.now() - lastPaymentTime > 10000)) p3 = 0.5;
    ...
    function handleInputState(el) {
        el.value.trim() ? el.classList.add('has') : el.classList.remove('has');
    }
    ...
    function fmtData(e) {
        ...
        e.value = v.replace(/^(\d{2})(\d)/,'$1/$2').replace(/^(\d{2})\/(\d{2})(\d)/,'$1/$2/$3');
    }
    function fmtCpf(e) {
        var v = e.value.replace(/\D/g,'');
        e.value = v.replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d)/,'$1.$2').replace(/(\d{3})(\d{1,2})$/,'$1-$2');
    }
    function fmtCnpj(e) {
        var v = e.value.replace(/\D/g,'');
        e.value = v.replace(/^(\d{2})(\d)/,'$1.$2').replace(/^(\d{2})\.(\d{3})(\d)/,'$1.$2.$3').replace(/\.(\d{3})(\d)/,'.$1/$2').replace(/(\d{4})(\d)/,'$1-$2');
    }
    function fmtTel(e) {
        var v = e.value.replace(/\D/g,'');
        e.value = v.length > 10
    ...
    function fmtCep(e) {
        var v = e.value.replace(/\D/g,'');
        e.value = v.replace(/^(\d{5})(\d)/,'$1-$2');
    }

    /* =========================================================
       CEP AUTOCOMPLETE + BUSCA FRETE
    ========================================================= */
    ...
    function fetchShippingRates(cep) {
        ...
        .then(function(data) {
            ...
            data.rates.forEach(function(rate) {
                ...
                card.innerHTML =
                    '<div class="outlined-head">' +
                        '<div>' +
                            '<div class="outlined-label">' + rate.title + '</div>' +
                        '</div>' +
                        '<div style="display:flex;align-items:center;gap:12px;">' +
                            '<span class="outlined-price">' + price + '</span>' +
        ...
        .catch(function() {
            container.innerHTML = '<p style="font-size:13px;color:var(--text-muted);">Erro ao calcular frete.</p>';
        });
    }
    ...
    function selectShipping(element, price, name) {
        document.querySelectorAll('.shipping-methods .outlined-card').forEach(function(c){
    ...
    function loadInstallments() {
        ...
        var totalText = totalEl.textContent.replace(/[R$\s]/g,'').replace(',','.');
        var total     = parseFloat(totalText) || config.subtotal || 0;
        var formKey   = document.getElementById('form_key') ? document.getElementById('form_key').value : '';

        fetch(config.installmentsUrl, {
            method: 'POST',
            headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
            body: 'total=' + total + '&form_key=' + encodeURIComponent(formKey)
        })
        .then(function(r){ return r.json(); })
        .then(function(data) {
            if (!data.success) return;
            var sel = document.getElementById('cc_installments');
            if (!sel) return;
            sel.innerHTML = '';
            ...
            // Atualiza texto de parcelas no resumo
            var it = document.getElementById('installment-text');
            if (it && data.installments.length > 0) {
                it.textContent = data.installments[0].label;
            }
        })
        .catch(function(){});
    }
    ...
    function applyCoupon() {
        ...
        if (code === 'ULTRA10') {
            ...
            alert('Cupom de R$ 10,00 aplicado com sucesso!');
        } else if (code !== '') {
    ...
    function calculateTotals() {
        ...
        document.querySelectorAll('.usc-qty').forEach(function(inp) {
            ...
            var unit = parseFloat(inp.dataset.price);
            var tot  = qty * unit;
            sub += tot;
        ...
        var pixDiscount = 0;
        var pixPct = config.pixDiscount || 5;
        if (currentPaymentMethod === 'pix') {
            pixDiscount = sub * (pixPct / 100);
            document.getElementById('discount-row').style.display = 'flex';
            document.getElementById('discount-label').textContent = 'Desconto Pix (−' + pixPct + '%)';
        } else {
            document.getElementById('discount-row').style.display = 'none';
        }

        var totalDiscount = appliedDiscountAmount + pixDiscount;
        var freight       = currentShippingPrice !== null ? currentShippingPrice : 0;
        var total         = Math.max(0, sub + freight - totalDiscount);
        ...
        document.getElementById('discount-val').textContent        = '−' + fmt(totalDiscount);
        document.getElementById('total-val').textContent           = fmt(total);

        var it = document.getElementById('installment-text');
        if (it && currentPaymentMethod !== 'card') {
            it.textContent = '1x de ' + fmt(total) + ' sem juros';
        }
    }
    ...
    function finalizeOrder() {
        ...
        var fields = ['email','senha','firstname','lastname','tax_document','resp_nome','resp_sobrenome',
                      'tax_document_pj','cnpj','inscricao_estadual','razao_social','telephone','postcode',
                      'street','number','complement','district','city','region_id','country',
                      'cc_number','cc_name','cc_expiry','cc_cid','cc_installments','coupon_code'];
        ...
        if (activeShipping) {
            postData += '&shipping_method=' + encodeURIComponent(activeShipping.dataset.shippingCode || '');
        }
    ...
    return {
        ...
        fmtTel:            fmtTel,
        fmtCep:            fmtCep,
        checkCepDisplay:   checkCepDisplay,
